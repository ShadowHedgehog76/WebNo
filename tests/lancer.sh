...
# le verdict se lit dans le titre : le mot « échec » figure aussi dans le
# code de la page, et le chercher partout le trouverait toujours
titre = re.search(r'<title>([^<]*)</title>', d)
sys.exit(0 if titre and titre.group(1).startswith('OK:') else 1)
"; then ECHECS=1; fi
done

echo
[ $ECHECS -eq 0 ] && echo "Tout est au vert." || echo "Des tests ont échoué."
exit $ECHECS
